...
from .expr_parser import parse_function, get_expr


def graphic_method(f, x_a, x_b, step=100):

    f_str = f

    fp_str = sympy.diff(get_expr(f))
...
def newton_raphson(x_a, f, m=1, tol=1e-6, err="abs", n_iter_max=1000):

    fp = parse_function(sympy.diff(get_expr(f)))
    f = parse_function(f)
...
def newton_raphson_multiples_raices(x_a, f, tol=1e-6, n_iter_max=1000):
